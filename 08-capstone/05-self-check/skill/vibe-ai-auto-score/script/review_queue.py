...
def collect_for_review(output_dir: Path, threshold: float = 0.7) -> list[dict]:
    ...
    for path in find_artifacts(output_dir):
        ...
        # also catch gate-based NEED_REVIEW (confidence_gate.verdict) — primary review signal
        gate = (data.get("aggregate", {}) or {}).get("confidence_gate", {}) or {}
        gate_verdict = gate.get("verdict")
        gate_need = (gate_verdict == "NEED_REVIEW") or (gate_verdict == "REJECT_FROM_AUTO_RANK")

        if need_review or gate_need or (confidence is not None and confidence < threshold):
            items.append({
                "timestamp": datetime.utcnow().isoformat(timespec="seconds") + "Z",
                "artifact": str(path),
                "confidence_score": confidence,
                "need_review": need_review,
                "reason": data.get("review_reasons", data.get("review_reason")) or (f"gate={gate_verdict}" if gate_need else ("auto: low confidence" if (confidence is not None and confidence < threshold) else "need_review=True")),
            })
